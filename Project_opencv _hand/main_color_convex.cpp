...
int main(int argc, const char** argv)
{
    ...
    while (1) {
       ...
       /*CvSeq* contours_s;// 메모리 블록에 형상을 향한 포인터 잡기
        CvSeq* result_s;// 형상의 포인터의 시퀀스 잡기
        CvMemStorage* storage = cvCreateChildMemStorage(0);//모든 형상들의 저장공간
        findContours(img_Gray, storage, &contours, sizeof(CvContour), CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE, cvPoint(0, 0));
        while(contours_s)
        {
          result_s= cvApproxPoly(contours, sizeof(CvContour), storage, CV_POLY_APPROX_DP, cvContourPerimeter(contours)*0.02, 0);
          */ 
        //오목점 또한 또하나의 볼록점으로 보고 하려했으나 실패
